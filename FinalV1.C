...
// Timer 0
   if (TMR0IF) {
	   ...
	   if(Program==1){
		   if((IR_program[x-1]-IR_program[x-2])>200){Break=1; IR_program[119]=IR_program[x-1]; IR_program[x-1]=0;}   // Break>200
		   else if((Time-IR_program[x-1])>3000){Break=1; IR_program[118]=1; IR_program[119]=Time-2700;}}
   TMR0 = -1000; // -400; -1000
   TMR0IF = 0;
}
...
//if(RB1) {IR_Program(120);}
if(RB4) {IR_output(0);}//RF_output(A2, A2_ON[18], A2_OFF[18]);A2 = (A2 + 1) % 2;} 	// RF
if(RB2) {IR_Program(0);}  											// IR Program Button
if(RB3) {IR_output(240);}   											// IR Transmit Button
